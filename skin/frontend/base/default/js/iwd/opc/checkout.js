...
IWD.OPC.Billing = {
		...
		init: function(){
			...
			$j_opc('input[name="shipping[create_account]"]').click(function(){
				if ($j_opc(this).is(':checked')){
					$j_opc('#register-customer-password').removeClass('hidden');
					$j_opc('input[name="shipping[customer_password]"]').addClass('required-entry');
					$j_opc('input[name="shipping[confirm_password]"]').addClass('required-entry');
				}else{
					$j_opc('#register-customer-password').addClass('hidden');
					$j_opc('input[name="shipping[customer_password]"]').removeClass('required-entry');
					$j_opc('input[name="shipping[confirm_password]"]').removeClass('required-entry');
		...
		validateForm: function(delay){
			...
			IWD.OPC.Billing.validate_timeout = setTimeout(function(){
				...
				if (valid){
					$j_opc('.opc-col-center').css({ display: "block" });
					IWD.OPC.Billing.save();
				}
				else{
					$j_opc('.opc-col-center').css({ display: "none" });
					if(mode != false)
		...
		setBillingForShipping:function(useBilling, skip_copy){
			if (useBilling==true){
				...
				$j_opc('#co-billing-form').addClass('hidden');
			}else{
				...
				$j_opc('#co-billing-form').removeClass('hidden');
			}
		...
		pushBilingToShipping:function(clearShippingForm){
			...
			$j_opc('#shipping-new-address-form input').not(':hidden, :input[type="checkbox"]').each(function(){
				var name = $j_opc(this).attr('name');
				var value = $j_opc(this).val();
				var shippingName =  name.replace( /shipping/ , 'billing');
				//var shippingName =  name.replace( /billing/ , 'shipping');

				$j_opc('#opc-address-form-billing input[name="'+shippingName+'"]').val(value);
			...
			$j_opc('#shipping-new-address-form input[name="shipping[street][]"]').each(function(indexBilling){
				...
				$j_opc('#opc-address-form-billing input[name="billing[street][]"]').each(function(indexShipping){
				 if (indexBilling==indexShipping){
				 $j_opc(this).val(valueAddress);
				 }
				 });
...
IWD.OPC.Shipping = {
		...
		save: function(){
			if ($j_opc('input[name="billing[use_for_shipping]"]').is(':checked')){
				IWD.OPC.Billing.pushBilingToShipping();
			}
...
IWD.OPC.Decorator = {
		...
		updateGrandTotal: function(response){
			...
			$j_opc('.orderreviewblock').html(response.fullreview);
		},
